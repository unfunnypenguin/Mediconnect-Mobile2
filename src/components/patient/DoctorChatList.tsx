...
type ChatItem = DoctorMessage | GroupedHealthcareAlerts;
...
const DoctorChatList: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchChatData() {
      ...
      setLoading(true);
      console.log('Fetching chat data for patient:', currentUser.id);
      ...
      try {
        ...
        if (sessionsError) {
          ...
          toast({
            title: "Error",
            description: "Failed to load doctor conversations.",
            variant: "destructive"
          });
          return;
        }

        const doctorChats: DoctorMessage[] = [];
        ...
        if (sessions?.length) {
          for (const session of sessions) {
            ...
            const { count: unreadCount } = await supabase
              ...
              .eq('session_id', session.id)
              .eq('sender_id', session.doctor_id)
              .eq('read', false);

            if (doctorProfile && lastMessage) {
              doctorChats.push({
        ...
        // Combine doctor chats and grouped alerts
        const allItems: ChatItem[] = [...doctorChats];
        if (groupedAlerts) {
